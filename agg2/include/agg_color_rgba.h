...
namespace agg
{
    ...
    struct rgba8
    {
        ...
        //--------------------------------------------------------------------
        AGG_INLINE void add(unsigned char* this_cover, const self_type& c, unsigned cover)
        {
			if (*this_cover == 0 || cover == cover_mask)
			{
				*this = c;
				*this_cover = cover;
				return;
			...
			calc_type const ac = calc_type(a)**this_cover;
			calc_type const cac = calc_type(c.a)*cover;
			a = (ac + cac + (total_cover>>1))/total_cover;
			calc_type const div = ac + cac;
			if (div)
			{
				r = (r*ac + c.r*cac)/div; // TODO: +(div>>1) ?
				g = (g*ac + c.g*cac)/div;
				b = (b*ac + c.b*cac)/div;
			}
			else
