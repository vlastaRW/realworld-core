...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    bool font_engine_win32_tt_base::create_font(const TCHAR* typeface_, 
                                                glyph_rendering ren_type)
    {
        if(m_dc)
        {
            unsigned len = 0;
			while (typeface_[len]) ++len;
            if(len > m_typeface_len)
            {
                delete [] m_signature;
                delete [] m_typeface;
                m_typeface  = new TCHAR [len + 32];
                m_signature = new char [len + 32 + 256];
                m_typeface_len = len + 32 - 1;
            }

            for (TCHAR* p = m_typeface; *p = *typeface_; ++typeface_) ++p;
