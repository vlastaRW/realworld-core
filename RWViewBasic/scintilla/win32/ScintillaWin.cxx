...
#ifdef _UNICODE
		{
			int nLen = ScintillaBase::WndProc(SCI_GETTEXTLENGTH, 0, 0);
			if (nLen)
			{
				char* pTmp = new char[nLen+1];
				ScintillaBase::WndProc(SCI_GETTEXT, nLen+1, (LPARAM)pTmp);
				int n = MultiByteToWideChar(CP_UTF8, 0, pTmp, -1, (LPWSTR)lParam, wParam);
				delete[] pTmp;
				return n;
			}
			if (wParam && lParam)
				*reinterpret_cast<LPWSTR>(lParam) = L'\0';
			return 0;
		}
#else
		return ScintillaBase::WndProc(SCI_GETTEXT, wParam, lParam);
#endif
	case WM_SETTEXT:
#ifdef _UNICODE
		{
			int nLen = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)lParam, -1, NULL, 0, NULL, NULL);
			if (nLen)
			{
				char* pTmp = new char[nLen];
				WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)lParam, -1, pTmp, nLen, NULL, NULL);
				ScintillaBase::WndProc(SCI_SETTEXT, 0, (LPARAM)pTmp);
				delete[] pTmp;
				return TRUE;
			}
			return ScintillaBase::WndProc(SCI_SETTEXT, 0, reinterpret_cast<LPARAM>(""));
		}
#else
		return ScintillaBase::WndProc(SCI_SETTEXT, wParam, lParam);
#endif
	case WM_GETTEXTLENGTH:
#ifdef _UNICODE
		{
			...
			if (nLen)
			{
				char* pTmp = new char[nLen+1];
				ScintillaBase::WndProc(SCI_GETTEXT, nLen+1, (LPARAM)pTmp);
				int n = MultiByteToWideChar(CP_UTF8, 0, pTmp, nLen, NULL, 0);
				delete[] pTmp;
				return n;
			}
			return 0;
...
#else
		return ScintillaBase::WndProc(SCI_GETTEXTLENGTH, wParam, lParam);
#endif
