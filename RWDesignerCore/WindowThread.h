...
class CWindowThread : public CMessageLoop
{
public:
	CWindowThread(CThreadCommand* a_pOrder, LCID a_tLocaleID,
	...
	HANDLE Start();
	void CreationFinished(HWND a_hMainWindow)
	{
		if (m_pOrder && m_pOrder->M_CreatedWindow())
			*m_pOrder->M_CreatedWindow() = a_hMainWindow;
		delete m_pOrder;
		m_pOrder = NULL;
	}
...
struct CConfigLock
{
	CConfigLock(CWindowThread* a_p) : m_p(a_p) { m_p->LockConfig(); }
