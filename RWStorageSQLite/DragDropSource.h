...
{
public:
	CDragDropSource() : m_dwLastEffect(DROPEFFECT_NONE), m_cde(0), m_rgde(NULL)
	{
	}
	~CDragDropSource()
	{
    ...
    // Operations
    bool Init(LPCTSTR a_pszDraggedFile, CLIPFORMAT a_eFormat, HWND a_hBlockCode)
	{
		_tcsncpy(m_szDraggedFile, a_pszDraggedFile, itemsof(m_szDraggedFile));
		m_szDraggedFile[itemsof(m_szDraggedFile)-1] = _T('\0');
		...
		cbyData += sizeof(TCHAR) * (1 + _tcslen(m_szDraggedFile));
		...
		_tcscpy(pszFilename, m_szDraggedFile);
		...
		// add blocker code
		fetc.cfFormat = a_eFormat;
		stg.hGlobal = GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, sizeof a_hBlockCode);
		*reinterpret_cast<HWND*>(GlobalLock(stg.hGlobal)) = a_hBlockCode;
		GlobalUnlock(stg.hGlobal);
		SetData(&fetc, &stg, TRUE);
...
private:
	...
	TCHAR m_szDraggedFile[MAX_PATH];
