...
#define DLSZ_DIVSIZE_X(factor) (DLSZ_SIZE_X|(((factor-1)&3)<<DLSZ_DIVSIZESHIFT_X))
#define DLSZ_MULDIVSIZE_X(nom, denom) (DLSZ_SIZE_X|(((((nom-1)&3)<<2)|((denom-1)&3))<<DLSZ_DIVSIZESHIFT_X))
#define DLSZ_DIVSIZE_Y(factor) (DLSZ_SIZE_Y|(((factor-1)&3)<<DLSZ_DIVSIZESHIFT_Y))
#define DLSZ_MULDIVSIZE_Y(nom, denom) (DLSZ_SIZE_Y|(((((nom-1)&3)<<2)|((denom-1)&3))<<DLSZ_DIVSIZESHIFT_Y))
#define DLSZ_DIVMOVE_X(factor) (DLSZ_MOVE_X|(((factor-1)&3)<<DLSZ_DIVMOVESHIFT_X))
#define DLSZ_MULDIVMOVE_X(nom, denom) (DLSZ_MOVE_X|(((((nom-1)&3)<<2)|((denom-1)&3))<<DLSZ_DIVMOVESHIFT_X))
#define DLSZ_DIVMOVE_Y(factor) (DLSZ_MOVE_Y|(((factor-1)&3)<<DLSZ_DIVMOVESHIFT_Y))
#define DLSZ_MULDIVMOVE_Y(nom, denom) (DLSZ_MOVE_Y|(((((nom-1)&3)<<2)|((denom-1)&3))<<DLSZ_DIVMOVESHIFT_Y))
...
// Data declarations and members
	enum
	{
		...
		// partial resizing
		DLSZ_DIVMOVEMASK_X = 0x00030000,
		DLSZ_MULMOVEMASK_X = 0x000c0000,
		DLSZ_DIVMOVESHIFT_X = 16,
		DLSZ_MULMOVESHIFT_X = 18,
		DLSZ_DIVSIZEMASK_X = 0x00300000,
		DLSZ_MULSIZEMASK_X = 0x00c00000,
		DLSZ_DIVSIZESHIFT_X = 20,
		DLSZ_MULSIZESHIFT_X = 22,
		DLSZ_DIVMOVEMASK_Y = 0x03000000,
		DLSZ_MULMOVEMASK_Y = 0x0c000000,
		DLSZ_DIVMOVESHIFT_Y = 24,
		DLSZ_MULMOVESHIFT_Y = 26,
		DLSZ_DIVSIZEMASK_Y = 0x30000000,
		DLSZ_MULSIZEMASK_Y = 0xc0000000,
		DLSZ_DIVSIZESHIFT_Y = 28,
		DLSZ_MULSIZESHIFT_Y = 30,
	...
	void DlgResize_Init(bool bAddGripper = true, bool bUseMinTrackSize = true, DWORD dwForceStyle = WS_CLIPCHILDREN)
	{
		...
		if(bAddGripper)
		{
			...
			if(!pT->GetDlgItem(ATL_IDW_STATUS_BAR).IsWindow())
			{
				...
				if(wndGripper.IsWindow())
				{
					...
					_AtlDlgResizeData data = { ATL_IDW_STATUS_BAR, DLSZ_MOVE_X | DLSZ_MOVE_Y | DLSZ_REPAINT | _DLSZ_GRIPPER, { rectCtl.left, rectCtl.top, rectCtl.right, rectCtl.bottom }, false };
					m_arrData.Add(data);
				}
			}
		}
		...
		for(int nCount = 1; !((pMap->m_nCtlID == -1) && (pMap->m_dwResizeFlags == 0)); nCount++, pMap++)
		{
			...
			else
			{
				...
				::GetClassName(ctl, szClass, 127);
				bool bSpecEdit = _tcscmp(szClass, _T("EDIT")) &&
					(ctl.GetWindowLong(GWL_STYLE) & (ES_AUTOHSCROLL | ES_LEFT | ES_MULTILINE)) == (ES_AUTOHSCROLL | ES_LEFT);
				_AtlDlgResizeData data = { pMap->m_nCtlID, pMap->m_dwResizeFlags | dwGroupFlag, { rectCtl.left, rectCtl.top, rectCtl.right, rectCtl.bottom }, bSpecEdit };
				m_arrData.Add(data);
			}
		}
...
// Implementation
	                               ...
	                               _AtlDlgResizeData* pDataPrev = NULL)
	{
		...
		else // no group
		{
			...
			{
				int nSizeNom = 1 + ((data.m_dwResizeFlags & DLSZ_MULSIZEMASK_X) >> DLSZ_MULSIZESHIFT_X);
				int nSizeDenom = 1 + ((data.m_dwResizeFlags & DLSZ_DIVSIZEMASK_X) >> DLSZ_DIVSIZESHIFT_X);
				int nMoveNom = 1 + ((data.m_dwResizeFlags & DLSZ_MULMOVEMASK_X) >> DLSZ_MULMOVESHIFT_X);
				int nMoveDenom = 1 + ((data.m_dwResizeFlags & DLSZ_DIVMOVEMASK_X) >> DLSZ_DIVMOVESHIFT_X);

				int nSizeDelta = ((data.m_dwResizeFlags & DLSZ_SIZE_X) != 0) ? ((cxWidth - m_sizeDialog.cx) * nSizeNom) / nSizeDenom : 0;
				int nMoveDelta = ((data.m_dwResizeFlags & DLSZ_MOVE_X) != 0) ? ((cxWidth - m_sizeDialog.cx) * nMoveNom) / nMoveDenom : 0;

				rectCtl.left = data.m_rect.left + nMoveDelta;
				rectCtl.right = data.m_rect.right + nMoveDelta + nSizeDelta;
			}
			...
			{
				int nSizeNom = 1 + ((data.m_dwResizeFlags & DLSZ_MULSIZEMASK_Y) >> DLSZ_MULSIZESHIFT_Y);
				int nSizeDenom = 1 + ((data.m_dwResizeFlags & DLSZ_DIVSIZEMASK_Y) >> DLSZ_DIVSIZESHIFT_Y);
				int nMoveNom = 1 + ((data.m_dwResizeFlags & DLSZ_MULMOVEMASK_Y) >> DLSZ_MULMOVESHIFT_Y);
				int nMoveDenom = 1 + ((data.m_dwResizeFlags & DLSZ_DIVMOVEMASK_Y) >> DLSZ_DIVMOVESHIFT_Y);

				int nSizeDelta = ((data.m_dwResizeFlags & DLSZ_SIZE_Y) != 0) ? ((cyHeight - m_sizeDialog.cy) * nSizeNom) / nSizeDenom : 0;
				int nMoveDelta = ((data.m_dwResizeFlags & DLSZ_MOVE_Y) != 0) ? ((cyHeight - m_sizeDialog.cy) * nMoveNom) / nMoveDenom : 0;

				rectCtl.top = data.m_rect.top + nMoveDelta;
				rectCtl.bottom = data.m_rect.bottom + nMoveDelta + nSizeDelta;
			}
		}
		...
		if((data.m_dwResizeFlags & (DLSZ_SIZE_X | DLSZ_SIZE_Y | DLSZ_MOVE_X | DLSZ_MOVE_Y | DLSZ_REPAINT | DLSZ_CENTER_X | DLSZ_CENTER_Y)) != 0)
		{
			...
			if (data.m_bFixEdit)
			{
				ULONG n1 = 0;
				ULONG n2 = 0;
				ctl.SendMessage(EM_GETSEL, reinterpret_cast<WPARAM>(&n1), reinterpret_cast<LPARAM>(&n2));
				ctl.SendMessage(EM_SETSEL, 0, 0);
				ctl.SendMessage(EM_SCROLLCARET, 0, 0);
				ctl.SendMessage(EM_SETSEL, n1, n2);
			}
		}
