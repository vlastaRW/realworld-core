...
struct IMenuTextProvider
{
	virtual LPCTSTR GetMenuItemText(UINT a_nID) = 0;
};
...
// Data members
	...
	IMenuTextProvider* m_pMenuTextProvider;
		...
		if(m_bMenuActive && (HIWORD(wParam) == MF_POPUP))
		{
			...
			for(int i = 0; i < nCount; i++)
			{
				...
				if(!bRet || (mii.fType & MFT_SEPARATOR))
					continue;
				LPCTSTR p = m_pMenuTextProvider ? m_pMenuTextProvider->GetMenuItemText(mii.wID) : NULL;
				if (p == NULL)
				{
