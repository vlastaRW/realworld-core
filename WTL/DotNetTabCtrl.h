...
public:
	...
	LRESULT OnSettingChange(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		WTL::CWindowDC dcWindow(m_hWnd);
		m_fScale = GetDeviceCaps(dcWindow, LOGPIXELSY)/96.0f;
		...
		m_settings.iIndent = 5*m_fScale+0.45f;
		m_settings.iPadding = 4*m_fScale+0.45f;
		m_settings.iMargin = 3*m_fScale+0.45f;
		m_settings.iSelMargin = 3*m_fScale+0.45f;
...
public:
	void DrawBackground(RECT rcClient, LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		else
		{
			...
			{
				rc.bottom = rc.top + int(2*m_fScale+0.5f);
			...
			{
				int nOrigTop = rc.top;
				rc.top = rc.bottom - int(1.75f*m_fScale+0.5f);
				dc.FillSolidRect(&rc, lpNMCustomDraw->clrBtnFace);
				...
				if(CTCS_NOEDGE != (dwStyle & CTCS_NOEDGE))
				{
					rc.top = nOrigTop;
	...
	void DrawItem_InitBounds(DWORD dwStyle, RECT rcItem, RECT& rcTab, RECT& rcText, int& nIconVerticalCenter)
	{
		if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
		{
			rcTab.top += int(1.75f*m_fScale+0.5f);
			rcTab.bottom -= int(2*m_fScale+0.5f);
			...
			nIconVerticalCenter = (rcItem.bottom + rcItem.top) / 2;
		}
		else
		{
			rcTab.top += int(2*m_fScale+0.5f);
			rcTab.bottom -= int(1.75f*m_fScale+0.5f);
	...
	void DrawItem_TabInactive(DWORD dwStyle, LPNMCTCCUSTOMDRAW lpNMCustomDraw, RECT& rcTab)
	{
		...
		else
		{
			...
			{
				// Important!  Be sure and keep within "our" tab area horizontally
				dc.MoveTo(rcTab.right-1, rcTab.top + int(2*m_fScale+0.5f));
				dc.LineTo(rcTab.right-1, rcTab.bottom - int(2*m_fScale+0.5f));
			}
			dc.SelectPen(penOld);
		}
	}
	...
	void DrawCloseButton(LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		//DrawFrameControl(dc, &m_rcCloseButton, DFC_CAPTION, DFCS_CAPTIONCLOSE|DFCS_FLAT|(ectcMouseDownL_CloseButton == (m_dwState & ectcMouseDown) ? DFCS_PUSHED : 0)|(ectcHotTrack_CloseButton == (m_dwState & ectcHotTrack) ? DFCS_HOT : 0));
		...
		const int sp = int(4*m_fScale+0.5f);
		...
		if (m_fScale > 1.6f)
		{
			dc.MoveTo(rcX.left+sp-1, rcX.top+sp+1);
			dc.LineTo(rcX.right-sp-2, rcX.bottom-sp);
			dc.LineTo(rcX.right-sp, rcX.bottom-sp);
		}
		...
		if (m_fScale > 1.6f)
		{
			dc.MoveTo(rcX.left+sp-1, rcX.bottom-sp);
			dc.LineTo(rcX.left+sp, rcX.bottom-sp);
			dc.LineTo(rcX.right-sp, rcX.top+sp);
		}
	...
	void DrawScrollButtons(LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		int halfSize = ((rcArrowRight.bottom-rcArrowRight.top)/3)-1;
		int horOff = ((rcArrowRight.right-rcArrowRight.left)-(halfSize+1))>>1;
		int verOff = ((rcArrowRight.bottom-rcArrowRight.top)-halfSize-halfSize)>>1;

		POINT ptsArrowRight[] = {
			{rcArrowRight.left+horOff, rcArrowRight.top+verOff},
			{rcArrowRight.left+horOff, rcArrowRight.top+verOff+halfSize+halfSize},
			{rcArrowRight.left+horOff+halfSize, m_rcScrollRight.top+verOff+halfSize},
			{rcArrowRight.left+horOff, rcArrowRight.top+verOff}
		};
		if(ectcOverflowRight != (m_dwState & ectcOverflowRight))
		{
		...
		POINT ptsArrowLeft[] = {
			{rcArrowLeft.left+horOff+halfSize, rcArrowLeft.top+verOff},
			{rcArrowLeft.left+horOff+halfSize, rcArrowLeft.top+verOff+halfSize+halfSize},
			{rcArrowLeft.left+horOff, rcArrowLeft.top+verOff+halfSize},
			{rcArrowLeft.left+horOff+halfSize, rcArrowLeft.top+verOff}
		};
		if(ectcOverflowLeft != (m_dwState & ectcOverflowLeft))
		{
...
public:
	...
	void CalcSize_CloseButton(LPRECT prcTabItemArea)
	{
		...
		int nButtonSizeX = int(15*m_fScale+0.5f);
		int nButtonSizeY = int(15*m_fScale+0.5f);
		...
		if (CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
		{
			m_rcCloseButton.top += int(3*m_fScale+0.5f);
			m_rcCloseButton.right -= int(3*m_fScale+0.5f);
		}
		else
		{
			m_rcCloseButton.top += int(1*m_fScale+0.5f);
			m_rcCloseButton.bottom -= int(2*m_fScale+0.5f);
			m_rcCloseButton.right -= int(2*m_fScale+0.5f);
		}
	...
	void CalcSize_ScrollButtons(LPRECT prcTabItemArea)
	{
		...
		if (CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
		{
			rcScroll.top += int(3*m_fScale+0.5f);
			if(0 == (dwStyle & CTCS_CLOSEBUTTON))
			{
				rcScroll.right -= int(3*m_fScale+0.5f);
			}
		}
		else
		{
			rcScroll.top += int(1*m_fScale+0.5f);
			rcScroll.bottom -= int(2*m_fScale+0.5f);
			if(0 == (dwStyle & CTCS_CLOSEBUTTON))
			{
				rcScroll.right -= int(2*m_fScale+0.5f);
			}
		}
