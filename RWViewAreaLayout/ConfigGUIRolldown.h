...
class ATL_NO_VTABLE CConfigGUIRolldownDlg :
	public CCustomConfigWndImpl<CConfigGUIRolldownDlg>,
	public CDialogResize<CConfigGUIRolldownDlg>
{
public:
	~CConfigGUIRolldownDlg()
	{
	...
	BEGIN_COM_MAP(CConfigGUIRolldownDlg)
		COM_INTERFACE_ENTRY(IChildWindow)
	END_COM_MAP()

	BEGIN_MSG_MAP(CConfigGUIRolldownDlg)
		CHAIN_MSG_MAP(CCustomConfigWndImpl<CConfigGUIRolldownDlg>)
		...
		CHAIN_MSG_MAP(CDialogResize<CConfigGUIRolldownDlg>)
	END_MSG_MAP()

	BEGIN_DLGRESIZE_MAP(CConfigGUIRolldownDlg)
		DLGRESIZE_CONTROL(IDC_TAB_LIST, DLSZ_SIZE_Y)
		DLGRESIZE_CONTROL(IDC_TAB_CFG, DLSZ_SIZE_X | DLSZ_SIZE_Y)
	END_DLGRESIZE_MAP()

	BEGIN_CONFIGITEM_MAP(CConfigGUIRolldownDlg)
	END_CONFIGITEM_MAP()
...
protected:
	...
	{
		CDialogResize<CConfigGUIRolldownDlg>::OnSize(a_uMsg, a_wParam, a_lParam, a_bHandled);
	...
	LRESULT OnOperationListRename(int a_idCtrl, LPNMHDR a_pNMHDR, BOOL& a_bHandled)
	{
		...
		{
			OLECHAR szNameID[64];
			_snwprintf(szNameID, itemsof(szNameID), L"%s\\%08x", CFGID_RD_ITEMS, pDispInfo->item.iItem);
	...
	LRESULT OnOperationListItemChanged(int a_idCtrl, LPNMHDR a_pNMHDR, BOOL& a_bHandled)
	{
		...
		else
		{
			...
			OLECHAR szNameID[64];
			_snwprintf(szNameID, itemsof(szNameID), L"%s\\%08x", CFGID_RD_ITEMS, nSel);
	...
	LRESULT OnClone(WORD a_wNotifyCode, WORD a_wID, HWND a_hWndCtl, BOOL& a_bHandled)
	{
		...
		if (nSel != -1)
		{
			...
			{
				CConfigValue cVal;
				CComBSTR cCFGID_RD_ITEMS(CFGID_RD_ITEMS);
				if (SUCCEEDED(M_Config()->ItemValueGet(cCFGID_RD_ITEMS, &cVal)))
				{
					CConfigValue cOrigName;
					M_Config()->ItemValueGet(CComBSTR(szNameID), &cOrigName);
					LONG nCount = cVal;
					_snwprintf(szNameID, itemsof(szNameID), L"%s\\%08x", CFGID_RD_ITEMS, nCount);
	...
	LRESULT OnDelete(WORD a_wNotifyCode, WORD a_wID, HWND a_hWndCtl, BOOL& a_bHandled)
	{
		...
		if (nSel != -1)
		{
			...
			if (SUCCEEDED(M_Config()->ItemValueGet(cCFGID_RD_ITEMS, &cVal)))
			{
				...
				{
					_snwprintf(szNameID1, itemsof(szNameID1), L"%s\\%08x", CFGID_RD_ITEMS, i);
					CComBSTR bstrNameID1(szNameID1);
					_snwprintf(szNameID2, itemsof(szNameID2), L"%s\\%08x", CFGID_RD_ITEMS, i+1);
				...
				M_Config()->ItemValuesSet(1, &(cCFGID_RD_ITEMS.m_str), CConfigValue(nCount - 1L));
		...
		if (nSel != -1)
		{
			...
			if (SUCCEEDED(M_Config()->ItemValueGet(cCFGID_RD_ITEMS, &cVal)))
			{
				...
				if ((nCount-1) > nSel)
				{
					OLECHAR szNameID1[64];
					OLECHAR szNameID2[64];
					_snwprintf(szNameID1, itemsof(szNameID1), L"%s\\%08x", CFGID_RD_ITEMS, nSel);
					CComBSTR bstrNameID1(szNameID1);
					_snwprintf(szNameID2, itemsof(szNameID2), L"%s\\%08x", CFGID_RD_ITEMS, nSel+1);
					CComBSTR bstrNameID2(szNameID2);
					CConfigValue cName1;
...
private:
	bool FillList()
	{
		...
		CConfigValue cVal;
		if (FAILED(M_Config()->ItemValueGet(CComBSTR(CFGID_RD_ITEMS), &cVal)))
			return false;
		LONG nCount = cVal;
		...
		{
			OLECHAR szNameID[64];
			_snwprintf(szNameID, itemsof(szNameID), L"%s\\%08x", CFGID_RD_ITEMS, i);
