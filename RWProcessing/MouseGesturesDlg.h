...
class CMouseGesturesDlg :
	public Win32LangEx::CLangDialogImpl<CMouseGesturesDlg>,
...
public:
	CMouseGesturesDlg(LCID a_tLocaleID, IConfig* a_pConfig) :
		Win32LangEx::CLangDialogImpl<CMouseGesturesDlg>(a_tLocaleID),
	...
	BEGIN_CTXHELP_MAP(CMouseGesturesDlg)
		CTXHELP_CONTROL_STRING(IDC_MG_GESTURES, L"[0409]Select a mouse gesture here and then use the controls on the right side of the dialog to configure an operation that will be executed when this gesture is drawn in raster editor using right mouse button.[0405]Vyberte gesto myši v tomto poli a poté použijte ovládací prvky v pravé části okna k nakofigurování operace, která bude provedena, když je gesto nakresleno pravým tlačítkem myši v rastrovém editoru.")
		CTXHELP_CONTROL_STRING(IDC_MG_OPERATION, L"[0409]Type of operation to execute when a gesture is recognized. Some operations have additional settings displayed below.[0405]Typ operace, která bude vykonána po rozpoznání gesta. Některé operace mají další nastavení zobrazeno dole.")
	END_CTXHELP_MAP()
...
public:
	...
	LRESULT OnDrawItem(UINT a_uMsg, WPARAM a_wParam, LPARAM a_lParam, BOOL& a_bHandled)
	{
		...
		if (a_wParam == IDC_MG_GESTURES && pDrawItem->CtlID == IDC_MG_GESTURES && pDrawItem->itemID < UINT(m_wndGestures.GetCount()))
		{
			...
			CComBSTR bstrItem;
			CMultiLanguageString::GetLocalized(pGesture->pszName, m_tLocaleID, &bstrItem);
			cDC.DrawText(bstrItem, -1, &rcText, DT_VCENTER|DT_LEFT|DT_SINGLELINE|DT_END_ELLIPSIS);
...
private:
	void SetActiveGesture(int a_nActiveGesture)
	{
		...
		m_pCfgWnd->ConfigSet(pCfg, ECPMFull);
	}
