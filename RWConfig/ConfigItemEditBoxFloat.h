...
class CConfigItemEditBoxFloat : public CConfigItemCtl
{
public:
	CConfigItemEditBoxFloat(CConfigItemParent const* a_pParentWnd, const RECT& a_rcPosition, LPCWSTR a_pszCfgID, UINT a_nCtlBaseID, LCID a_tLocaleID, bool a_bAdjustHeight) :
	...
	virtual ~CConfigItemEditBoxFloat()
	{
	}
	...
	inline static CConfigItemCtl* CreateControl(CConfigItemParent const* a_pParentWnd, const RECT& a_rcPosition, LPCWSTR a_pszCfgID, UINT a_nCtlBaseID, LCID a_tLocaleID, bool a_bAdjustHeight)
	{
		...
		if (cValue.TypeGet() == ECVTFloat)
		{
			return new CConfigItemEditBoxFloat(a_pParentWnd, a_rcPosition, a_pszCfgID, a_nCtlBaseID, a_tLocaleID, a_bAdjustHeight);
		}
		else
		{
			return NULL;
	...
	virtual void Update()
	{
		...
		ATLASSERT(cValue.TypeGet() == ECVTFloat);
		TCHAR buf[32];
		_stprintf(buf, _T("%g"), cValue.operator float());
	...
	virtual void EventReflected(WORD a_wNotifyCode)
	{
		if (a_wNotifyCode == EN_KILLFOCUS)
		{
			...
			if (cValue.TypeGet() == ECVTFloat)
			{
				...
				TCHAR* pEnd = szTmp;
				float fTmp = static_cast<float>(_tcstod(szTmp, &pEnd));
				if (*pEnd == _T('\0') || *pEnd == _T(' ') || *pEnd == _T('\t'))
				{
					cValue = fTmp;
				}
			}
